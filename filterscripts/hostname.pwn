...
public PiarTextToAll(server_name[], site_server[], forum_server[])
{
    ...
    SendClientMessageToAll(-1, "          \tБонусная система {F6AB2F}/bonus{FFFFFF}");
    SCMALLF(-1, "          \tПополнить счёт: {F6AB2F}%s", site_server);
    SCMALLF(-1, "          \tНаш форум: {F6AB2F}%s", forum_server);
    SendClientMessageToAll(-1, "  ");
    SCMALLF(0xF6AB2FFF, "{F6AB2F}[!]{ffffff}=-=-=-=-=-=-=-=-=-=-=-=-=-=[ {F6AB2F}Arizona %s{FFFFFF} ]=-=-=-=-=-=-=-=-=-=-=-=-=-={F6AB2F}[!]{ffffff}", server_name);
}
...
{    
    if(WRITE_ACTION_SERVER)
    {
        ...
        LOG_BASE = mysql_connect("d99961hc.beget.tech", "d99961hc_logs", "d99961hc_logs", "%v4yOXPM");
